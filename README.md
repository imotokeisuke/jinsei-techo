...
## 更新履歴（マインドマップ改善／育つ木コレクション）

- マインドマップの円と文字が重なる問題を修正しました。大カテゴリーのラベルも円の外側に表示するようにし、カテゴリー数が増えるほど自動的にキャンバスを広げるようにしました
- マインドマップを指2本でのピンチズーム、ドラッグでのパン、マウスホイールでの拡大縮小に対応しました（右下に−／リセット／＋ボタンも追加）
- 「育つ木」ウィジェットをタップすると「育つ木コレクション」が開き、たね〜オレンジが実る木までの全6段階を見られるようになりました。まだ日記の件数が届いていない段階は「？？？」のシークレット表示になり、条件を満たすと自動的に解禁されます
