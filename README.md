...
## 更新履歴（日記→エピソード同時登録／エピソードの写真）

- 日記の作成・編集画面に「＋この内容をエピソードにも登録する」ボタンを追加しました。オンにすると大カテゴリー（人生手帳・エピソード共通のもの）を選ぶ欄が出てきて、保存すると同じ日付・タイトル・本文でエピソードにも登録されます
- エピソードに写真を追加できるようになりました
  - 複数枚追加可能。追加した写真は自動的に縮小・圧縮してから保存されるため、通信量やデータベースの容量を抑えられます（長辺1000px程度、JPEG）
  - 一覧・編集画面どちらにも小さくサムネイル表示されます
  - ※Realtime Databaseに直接保存する方式のため、大量の写真や非常に高解像度な写真を大量登録するのには向きません。写真を中心に使う場合は、Firebase Storageの利用への切り替えもご提案できるので、必要であればお知らせください
